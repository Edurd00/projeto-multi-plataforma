...
export const ImageUpload = {
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex-grow space-y-2">
            ...
            <div class="flex gap-2">
              <button type="button" onclick="document.getElementById('input-${id}').click()" class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-[10px] font-black uppercase tracking-widest py-2 px-3 rounded-lg transition">
                Selecionar
              </button>
              <button type="button" id="remove-${id}" class="${currentUrl ? 'flex' : 'hidden'} bg-red-50 hover:bg-red-100 text-red-500 text-[10px] font-black uppercase tracking-widest py-2 px-3 rounded-lg transition items-center gap-1">
                ✕ Remover
              </button>
            </div>
            <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">PNG, JPG ou WEBP. Máx 2MB.</p>
  ...
  bindEvents(id, onUploadComplete) {
    ...
    if (removeBtn) {
        removeBtn.onclick = () => {
            urlInput.value = '';
            removeBtn.classList.replace('flex', 'hidden');
